...
int BinarySearchTreeX<Comparable>::height() const
{
   ...
   return height(root)-1; //as height count includes the root so we subtract 1
}
...
{
    if (find(possibleAncestor,root)){
        if (find(x,find(possibleAncestor,root))){
            return true;
        }
    }
...
{
   if (!(t->left && t->right)){ //if either of left or right is NULL which means it is not full, then we have to increase level
    return 1;
   }
   else{
    if (highestFull(t->left) > highestFull(t->right)){
        return 1 + highestFull(t->right);
    }
    else
        return 1 + highestFull(t->left);
   }
...
int BinarySearchTreeX<Comparable>::deepestFull(BinaryNodeX<Comparable> *t) const
{
  ...
  return height() - highestFull(t);  // to be written by student
}
